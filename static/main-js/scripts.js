...
(function() {
  ...
  const thumbnails = modal.querySelectorAll('.thumbnail-item');
    ...
    // Open modal on image click - explicitly show Bootstrap modal then update image
    imageContainers.forEach((container) => {
      container.addEventListener('click', function(e) {
        e.preventDefault();

        // guard: ensure index is a number
        const idx = parseInt(this.dataset.index);
        if (isNaN(idx)) return;
        currentImageIndex = idx;

        // Explicitly show the Bootstrap modal to ensure markup and transitions are applied
        try {
          const bsModal = new bootstrap.Modal(document.getElementById('galleryModal'));
          bsModal.show();
        } catch (err) {
          // If Bootstrap not available or error occurs, continue — update will still attempt to run
          console.warn('Bootstrap modal show failed:', err);
        }

        // Small timeout to allow modal to become visible before loading image (prevents visual clipping)
        setTimeout(() => {
          updateModalImage();
        }, 60);
      });
  ...
  function updateModalImage() {
    ...
    img.onload = function() {
      ...
      img.onerror = function() {
      if (loader) loader.style.display = 'none';
      if (window.ZorpidoNotify && window.ZorpidoNotify.notify) {
        window.ZorpidoNotify.notify('Failed to load image', 'error');
      } else {
   ...
   */
  function setupActionButtons() {
    // Download and Share functionality removed for Glimpses modal per UX update.
    // This function intentionally left blank to avoid errors from removed DOM elements.
    return;
  }
   ...
   */
  /**
   * NotificationManager - modular toast-style notifications
   * Usage: window.ZorpidoNotify.notify('Message', 'success', {timeout:3000})
   */
  (function() {
    const containerId = 'zorpido-toast-container';
    ...
    function createToast(message, type = 'info', opts = {}) {
      ...
      toast.className = `zorpido-toast toast-${type}`;
      toast.tabIndex = 0;

      const inner = document.createElement('div');
      inner.className = 'zorpido-toast-inner';

      const msg = document.createElement('div');
      msg.className = 'zorpido-toast-message';
      msg.innerHTML = message;

      const close = document.createElement('button');
      close.className = 'zorpido-toast-close';
      ...
      progress.className = 'zorpido-toast-progress';
      ...
      let timeoutId = setTimeout(dismiss, remaining);
      rafId = requestAnimationFrame(updateProgress);
      ...
      // accessibility: allow Esc to close when focused
    ...
    // Public API
    window.ZorpidoNotify = {
      notify: function(message, type = 'info', opts = {}) {
        return createToast(message, type, opts);
      }
    };
  })();
