...
from .models import CreditTransaction, Expense, Register, Payable
...
def add_expense(request):
    """Add a new expense to the current register"""
    try:
        ...
        # Get active register
        register = Register.objects.filter(is_open=True).order_by('-opened_at').first()
        if not register:
            return JsonResponse({'success': False, 'error': 'No active register found'})

        # Resolve or create seller
        seller_obj = None
        if seller_id:
            try:
                seller_obj = Seller.objects.get(id=int(seller_id))
            except Exception:
                seller_obj = None
        elif seller_name:
            seller_obj, _ = Seller.objects.get_or_create(name=seller_name)

        # Validation: if paid, ensure payment_mode provided
        if payment_status == 'paid' and not payment_mode:
            return JsonResponse({'success': False, 'error': 'Payment mode required for paid expenses'}, status=400)
        ...
        # If expense is paid, update register totals immediately
        if payment_status == 'paid':
            register.expenses_total = (register.expenses_total or Decimal('0.00')) + amount
            register.save()
        else:
            # Unpaid -> create payable (avoid duplicates)
            try:
                if not Payable.objects.filter(related_expense=expense).exists():
                ...
                logging.exception('Failed to create payable for expense %s', expense)
...
def pos_overview(request):
    ...
    context = {
        ...
        'today_expenses': today_expenses.filter(payment_status='paid'),
        'today_expenses_total': today_expenses.filter(payment_status='paid').aggregate(total=Sum('amount'))['total'] or Decimal('0.00'),
...
def export_pos_overview_pdf(request):
    ...
    # Only include paid expenses in totals
    expenses_qs = expenses_qs.filter(payment_status='paid')
    ...
    # Expenses detail (small table)
    story.append(Paragraph('Expenses', styles['Heading2']))
    exp_table = [['Date', 'Description', 'Amount (NPR)']]
    for e in expenses_qs.order_by('created_at')[:500]:
        exp_table.append([e.created_at.strftime('%Y-%m-%d'), (e.description or '')[:60], f"{(e.amount or Decimal('0.00')):.2f}"])
    et = Table(exp_table, hAlign='LEFT', colWidths=[30*mm, 100*mm, 40*mm])
...
def pos_dashboard(request):
    """Render POS dashboard with credit/expense totals and customer list"""
    today = timezone.localdate()
    start_of_day = timezone.make_aware(timezone.datetime.combine(today, timezone.datetime.min.time()))
    end_of_day = timezone.make_aware(timezone.datetime.combine(today, timezone.datetime.max.time()))

    total_credit = User.objects.filter(user_type='customer').aggregate(total=Sum('credit_balance'))['total'] or Decimal('0.00')
    expenses = Expense.objects.filter(created_at__gte=start_of_day, created_at__lte=end_of_day)
    total_expenses = expenses.aggregate(total=Sum('amount'))['total'] or Decimal('0.00')

    categories = Category.objects.all()
    customers = User.objects.filter(user_type='customer').order_by('full_name')
    sellers = Seller.objects.all().order_by('name')
    staff_users = User.objects.filter(is_active=True).filter(Q(is_staff=True) | Q(user_type='staff')).order_by('full_name')
...
def api_overview_data(request):
    ...
    # Expenses - only paid expenses count towards totals
    expenses_qs = Expense.objects.filter(created_at__gte=start_dt, created_at__lte=end_dt, payment_status='paid')
    ...
    while cur <= today:
        ...
        day_exp = Expense.objects.filter(created_at__gte=day_start, created_at__lte=day_end, payment_status='paid').aggregate(total=Sum('amount'))['total'] or Decimal('0.00')
        sales_data.append(float(day_sales))
        expenses_data.append(float(day_exp))
        cur = cur + timezone.timedelta(days=1)

    summary = {
        'total_sales': float(sales_total),
        'total_expenses': float(expenses_total),
        'net_profit': float((sales_total or Decimal('0.00')) - (expenses_total or Decimal('0.00'))),
...
def inventory_management(request):
    ...
    low_stock_items = MenuItem.objects.filter(stock_quantity__lte=F('low_stock_threshold')).order_by('stock_quantity')
    all_items = MenuItem.objects.all().select_related('category').order_by('category__name', 'name')
...
def seller_detail(request, seller_id):
    ...
    # Show purchase history grouped by expense
    purchases = seller.expenses.select_related('register').order_by('-created_at')[:200]
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if payable_id:
                ...
                # Update related expense status if linked
                try:
                    if p.related_expense:
                        p.related_expense.payment_status = 'paid'
                        p.related_expense.save()
                except Exception:
                    logging.exception('Failed to update related expense for payable %s', p.id)
...
def register_detail(request, reg_id):
    ...
    # Expenses attached to register
    expenses = register.expenses.all().order_by('-created_at')
