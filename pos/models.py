...
class Register(models.Model):
    ...
    def recalculate_totals(self):
        """Recalculate totals from related orders and expenses."""
        from orders.models import Order
        orders = Order.objects.filter(status='completed', completed_at__gte=self.opened_at)
        cash = orders.filter(payment_method='cash').aggregate(s=Sum('paid_amount'))['s'] or Decimal('0.00')
        credit = orders.filter(payment_method='credit').aggregate(s=Sum('paid_amount'))['s'] or Decimal('0.00')
        qr = orders.filter(payment_method='qr').aggregate(s=Sum('paid_amount'))['s'] or Decimal('0.00')
        # Only include paid expenses when computing totals
        expenses = self.expenses.filter(payment_status='paid').aggregate(s=Sum('amount'))['s'] or Decimal('0.00')

        self.cash_total = cash
        self.credit_total = credit
        self.qr_total = qr
        self.expenses_total = expenses
        # closing balance can be computed as opening + (cash_total + credit_total + qr_total) - expenses_total
        self.closing_balance = (self.opening_balance + cash + credit + qr - expenses)
        self.save()


class Expense(models.Model):
    register = models.ForeignKey(Register, on_delete=models.CASCADE, related_name='expenses')
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    description = models.CharField(max_length=255, blank=True)
    # Optional seller for the expense (supplier/vendor)
    seller = models.ForeignKey('Seller', on_delete=models.SET_NULL, null=True, blank=True, related_name='expenses')
    PAYMENT_MODE_CHOICES = [
        ('cash', 'Cash'),
        ('qr', 'QR'),
    ]
    ...
    def __str__(self):
        return f"Expense {self.amount} on {self.created_at}"
...
class Payable(models.Model):
    ...
    seller = models.ForeignKey('Seller', on_delete=models.SET_NULL, null=True, blank=True, related_name='payables')
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    related_expense = models.OneToOneField(Expense, on_delete=models.CASCADE, related_name='payable', null=True, blank=True)
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default='pending')
    remaining_amount = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0.00'))
    paid_at = models.DateTimeField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class Seller(models.Model):
    ...
    def total_purchases(self):
        from django.db.models import Sum
        # Sum only paid expenses as purchases
        s = self.expenses.filter(payment_status='paid').aggregate(total=Sum('amount'))['total'] or Decimal('0.00')
        return s
