...
def customer_profile(request):
	...
	if request.method == 'POST':
		form = ProfileUpdateForm(request.POST, request.FILES, instance=request.user)
		if form.is_valid():
			# Apply cleaned data explicitly to avoid issues when rendering custom inputs
			cd = form.cleaned_data
			user = request.user
			# Update scalar fields
			for f in ['full_name', 'email', 'phone', 'date_of_birth', 'location']:
				if f in cd:
					setattr(user, f, cd.get(f))
...
def customer_dashboard(request):
	...
	credit_balance = request.user.credit_balance
	loyalty_points = request.user.loyalty_points
	# Compute lifetime earned points from loyalty transaction history (type='earned')
	loyalty_points_earned = 0
	try:
		from loyalty.models import LoyaltyTransaction
		agg = LoyaltyTransaction.objects.filter(customer=request.user, transaction_type='earned').aggregate(total=Sum('points'))
		loyalty_points_earned = int(agg['total'] or 0)
	except Exception:
		# Fallback to the user model field if loyalty app or model not available
		loyalty_points_earned = int(getattr(request.user, 'total_points_earned', 0) or 0)
	recent_credits = request.user.credit_transactions.all()[:10]
	recent_loyalty = request.user.loyalty_transactions.all()[:10]  # Ensure loyalty history is passed
	return render(request, 'users/dashboard.html', {
		'credit_balance': credit_balance,
...
def dashboard_status(request):
	"""Return JSON with minimal status for realtime polling."""
	from django.http import JsonResponse
	# Try to compute lifetime earned points from loyalty history
	loyalty_points_earned = 0
	try:
		from loyalty.models import LoyaltyTransaction
		agg = LoyaltyTransaction.objects.filter(customer=request.user, transaction_type='earned').aggregate(total=Sum('points'))
